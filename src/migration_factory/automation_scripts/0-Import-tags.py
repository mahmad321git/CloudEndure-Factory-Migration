...
def data_validation(data, servers):
    ...
    for row in data:
        for key in keys:
            if key not in row:
                print("ERROR: " + key +
                      " tag value is missing for server " + row['Name'])
                sys.exit(4)
            if row[key] is None:
                print("ERROR: " + key +
                      " tag value is missing for server " + row['Name'])
            ...
            if row[key] == row[key].strip() == "":
                print("ERROR: " + key + " tag for server " +
                      row['Name'] + " is empty")
                sys.exit(7)
    ...
    for server in server_list:
        ...
        for s in servers:
            if (server.lower() == s['server_name'].lower()):
                match = True
        if (not match):
            print("ERROR: Server " + server +
                  " doesn't exist in the migration factory")
            sys.exit(1)
...
def uploading_data(data, token, UserHOST):
    ...
    for row in data:
        ...
        for server in servers:
            if row['Name'].strip().lower() == server['server_name'].strip() \
                    .lower():
        ...
        r = requests.put(UserHOST + serverendpoint + '/' + server_id,
                         headers=auth, data=json.dumps(update_server_tags))
        if r.status_code == 200:
            print("Tags for server " + row['Name'] +
                  " updated in the migration factory")
        else:
            print("ERROR: updating tags for server " +
                  row['Name'] + " failed : " + r.text + ".......")
...
def main(arguments):
    ...
    parser.add_argument('--Intakeform', default='0-import-tags.csv')
    ...
    _ = uploading_data(data, token, UserHOST)
