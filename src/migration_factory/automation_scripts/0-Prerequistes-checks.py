...
SKIP THIS STEP IF CE AGENT HAS NOT BEEN SETUP.

NOTE: Prerequisites check only works after the installation of CEAgent on
      at least one source server.
"""
...
def check_windows(Servers_Windows, CEServerIP, Domain_User,
    ...
    for s in Servers_Windows:
        ...
        s_result = {}
        final = ""
        command = "Invoke-Command -ComputerName " + \
            s["server_fqdn"] + \
            " -FilePath " + \
            "..\\migration_factory\\automation_scripts\\" + \
            "0-Prerequistes-Windows.ps1" + \
            " -ArgumentList " + CEServerIP
        if ip_address_helper.is_valid_ip_address(s['server_fqdn']):
            command += " -Credential (New-Object System.Management.Automation" \
                ".PSCredential(\"" + Domain_User + \
        ...
        if 'FreeSpace' in s_result:
            print(r" 3GB C:\ Free Space     : " + s_result['FreeSpace'])
        print("")
...
def check_freespace(ssh, dir, min,  s_result):
    ...
    if len(ssh_err) > 0:
        ...
        if "final_result" in s_result:
            s_result["final_result"] = s_result["final_result"] + \
                "FreeSpace" + str(min) + ","
        else:
...
def check_linux(user_name, pass_key, has_key, Servers_Linux, CEServerIP):
    linux_results = []
        ...
        if "SSH22" not in s_result["final_result"]:
            ...
            if "SUDO" not in s_result["final_result"]:
                ...
                check_tcp_connectivity(
                    ssh, 'console.cloudendure.com', '443', s_result)
...
def print_results(label, results, UserHOST, token):
        ...
        if 'final_result' in result:
            if result['final_result'] == "":
                if 'error' not in result:
                    ...
                    serverattr = {
                        "migration_status": "Pre-requisites check : Passed"}
                    _ = requests.put(UserHOST + serverendpoint + '/' +
                                     result['server_id'], headers={
                                         "Authorization": token}, data=json
                                     .dumps(serverattr))
    ...
    for result in results:
        if 'final_result' not in result:
            print("     " + result[
                'server_name'] + " : Unexpected error, please check error "
                "details")
            serverattr = {
                "migration_status": "Pre-requisites check : Failed - "
                "Unexpected error"}
            _ = requests.put(
            ...
            if result['final_result'] != "":
                ...
                _ = requests.put(
                    UserHOST + serverendpoint + '/' + result['server_id'],
                    headers={"Authorization": token}, data=json
                    .dumps(serverattr))
    print("")
...
def main(arguments):
    ...
    print("****************************")
    Servers_Windows, Servers_Linux = ServerList(
        args.Waveid, token, UserHOST, args.CloudEndureProjectName)
    ...
    if len(Servers_Windows) > 0:
        ...
        windows_results = check_windows(
            Servers_Windows, args.CEServerIP, windows_user_name,
            windows_pass_key)
    ...
    if len(Servers_Linux) > 0:
        ...
        linux_results = check_linux(user_name, pass_key, has_key, Servers_Linux,
                                    args.CEServerIP)
